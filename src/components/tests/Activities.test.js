...
describe('<Activities />', () => {
   it('renders', () => {
       const wrapper = shallow(<Activities bufferzone={true} parentId={1}
           notify={mockNotify} pofTreeUpdate={mockPofTreeUpdate}
           deleteActivityFromBuffer={mockDeleteActivityFromBuffer} 
           deleteActivityFromEvent={mockDeleteActivityFromEvent}
           className="activity-list"
           activities={[testActivity]}
           />)
       expect(wrapper.find({className:"activity-list"}).children().length).to.equal(1) 
   })
   it('delete function calls deleteactivity and pofTreeUpdate', () => {
       ...
       activity.instance().props.deleteActivity(testActivity).then( () => {
